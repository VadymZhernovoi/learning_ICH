...
matches = re.finditer(r"\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4}|\d{2}\.\d{2}\.\d{4}", text)
for match in matches:
    print(match.group())

print("список findall:")
matches = re.findall(r"\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4}|\d{2}\.\d{2}\.\d{4}", text)
for match in matches:
    print(match)
...
print()
print("findall:")
matches = re.findall(r"[^,;/\s]+", tag_input)
print(matches)
# Удалить лишние пробелы и пустые значения.
matches = [tag.strip() for tag in re.findall(r"[^,;/\s]+", tag_input) if tag] # хотя это лишнее - '\s' и '+' и так не дают лишних пробелов и пустых значений
print(matches)
